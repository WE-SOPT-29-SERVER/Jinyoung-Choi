...
const convertSnakeToCamel = require('../lib/convertSnakeToCamel.js');
...
const getUserById = async (client, userId) => {
  const { rows } = await client.query(
    ...
    //WHERE id = '${userId}' 처럼 ''로 감싸줘야함
    // $1에는 배열의 첫번째 변수가, $2에는 배열의 두 번째 변수... 이런 식으로 쿼리문에 변수가 들어가게 됩니다!
    [userId], //이런 식으로 배열 적어서 알맞은 type으로 넣을 수 있음. mysql은 ?로
  );
...
//새 객체 {}만들고, 왼쪽에서 오른쪽으로 가면서 겹치는 key가 있으면 바꾼다
  const { rows } = await client.query(
    `
    UPDATE "user" u
